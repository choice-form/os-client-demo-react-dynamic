...
## 插件中引用第三方库
+ 如果插件程序`src/plugin`中引用了第三方库,记得到`build.config.js`中配置一下`splitChunks`中配置一下,将这个第三方库单独打包,否则会导致许多入口插件的chunk重复包含这个库,不利于动态加载,具体说明请看`build.config.js`中的配置
...
+ 项目中我们默认添加了一些额外的`tslint`检查规则,来保证更持有的可维护性.
  - 要求所有的方法都显示指定返回类型
